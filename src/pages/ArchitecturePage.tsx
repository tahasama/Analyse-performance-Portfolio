...
export default function ArchitecturePage() {
  ...
  return (
    <div>
      ...
      <section className="max-w-5xl mx-auto px-6 pt-16 pb-12 md:pt-24 md:pb-24">
      ...
      {/* HORIZON completes the dark introduction before the light
          architecture explanation begins. */}
      <section className="max-w-5xl mx-auto px-6 pt-10 pb-16 border-t border-border">
      ...
      {/* Light -- the architecture introduction and standard sample. */}
      <div className="theme-editorial bg-background text-foreground">
        <section className="max-w-5xl mx-auto px-6 pb-10 pt-16">
          ...
          <p className="callout text-sm text-foreground/85 max-w-2xl">
            ...
            This page shows the architecture, its pillars, handshakes,
            principles, and the structure of its formal standard. Metric
            formulas, Health Score weighting, thresholds, routing, escalation
            logic, and simulation mechanics remain in the controlled
            specification.
          </p>
        </section>
      ...
      <section className="px-6 py-10 lg:py-8">
        <div className="mobile-diagram-prompt mx-auto max-w-sm items-center justify-center gap-3 border-y border-border py-5 text-center">
          <span
            className="shrink-0 font-mono text-3xl leading-none text-accent"
            aria-hidden="true"
          >
            ↻
          </span>
      ...
      <div className="theme-editorial bg-background text-foreground">
        <section className="max-w-5xl mx-auto px-6 pb-16 pt-16">
          <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start mb-10">
            <div>
              ...
              <p className="text-lg text-muted-foreground leading-relaxed">
                ...
                internal standard. It defines the framework's authority,
                governance model, six instruments, performance model, data
                architecture, thresholds and enforcement.
              </p>
            </div>
            ...
            <div className="relative w-full max-w-xs">
              ...
              <div className="relative border border-border bg-card px-6 py-8">
                ...
                <p className="font-serif text-2xl text-foreground mb-2">
                  Version {standardMeta.version}
                </p>
                <p className="text-sm text-muted-foreground">
                  Version-controlled internal standard
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-2">
            {[standardThemes.slice(0, 3), standardThemes.slice(3)].map(
              (column, columnIndex) => (
                <ul
                  key={columnIndex}
                  className={
                    columnIndex === 0
                      ? "divide-y divide-border lg:pr-8"
                      : "divide-y divide-border border-t border-border lg:border-t-0 lg:border-l lg:pl-8"
                  }
                >
                  {column.map((theme, i) => (
                    <li key={theme.label} className="flex gap-4 py-4">
                      <span className="shrink-0 pt-0.5 font-mono text-sm text-accent">
                        {String(columnIndex * 3 + i + 1).padStart(2, "0")}
                      </span>
                      <div>
