  ...
  return (
    <div>
      ...
      <section className="max-w-5xl mx-auto px-6 pt-24 pb-20">
        ...
        >
          &ldquo;The broader architectural / research work that maps the documentation profession and its
          surrounding governance disciplines.&rdquo;
        </p>
        <p
      ...
      <div className="theme-editorial bg-background text-foreground">
        <section className="max-w-5xl mx-auto px-6 py-16">
          <p className="section-label mb-3">Chapter 0 — How This Is Organized</p>
          <h2 className="font-serif text-2xl text-foreground mb-3">{chapter0.title}</h2>
          <p className="text-muted-foreground leading-relaxed max-w-2xl mb-8">{chapter0.gloss}</p>
          <ul className="divide-y divide-border border-y border-border">
            {dbok.architecturalPatterns.map((pattern, i) => (
              <li key={pattern.term} className="flex gap-4 py-4">
                <span className="shrink-0 pt-0.5 font-mono text-sm text-accent">{String(i + 1).padStart(2, "0")}</span>
                <div>
                  <p className="text-base font-medium text-foreground mb-1">{pattern.term}</p>
      ...
      <div className="theme-editorial bg-background text-foreground">
        <section className="max-w-5xl mx-auto px-6 pb-24 pt-16">
          ...
          <p className="text-muted-foreground leading-relaxed max-w-2xl mb-8">
            Every chapter is decomposed into the same eight kinds of entity, so disciplines stay comparable
            instead of each reading like its own taxonomy.
          </p>
          <div className="grid sm:grid-cols-2 gap-px bg-border">
            {dbok.knowledgeTypes.map((type) => (
              <div key={type.name} className="bg-background p-5">
                ...
                <p className="text-sm text-foreground/75 leading-relaxed">{type.description}</p>
              </div>
            ))}
          </div>
        </section>
