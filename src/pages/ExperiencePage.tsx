...
export default function ExperiencePage() {
  const { positioningStatement, summary, roles, education, skills, contact } = experience;
    ...
    <div>
      ...
      <section className="max-w-5xl mx-auto px-6 pb-16">
        ...
        <div className="relative pl-8">
          <div className="absolute left-0 top-1 bottom-1 w-px bg-border" aria-hidden="true" />
          <div className="space-y-12">
            {roles.map((role) => (
              <div key={`${role.organization}-${role.period}`} className="relative">
                <span className="absolute -left-8 top-1.5 h-2 w-2 rounded-full bg-accent ring-4 ring-background" />
                <div className="flex flex-wrap items-baseline justify-between gap-2 mb-1">
                  <h3 className="font-serif text-xl text-foreground">{role.role}</h3>
                  <span className="text-sm text-muted-foreground tabular-nums">{role.period}</span>
                </div>
                <p className="text-sm font-medium text-accent mb-3">{role.organization}</p>
      ...
      <section className="max-w-5xl mx-auto px-6 pb-16 border-t border-border pt-12">
        ...
        {education.map((e) => (
          <div key={e.credential}>
            <h3 className="font-serif text-xl text-foreground mb-1">{e.credential}</h3>
      ...
      <section className="max-w-5xl mx-auto px-6 pb-16 border-t border-border pt-12">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {skills.map((group) => (
            <div key={group.title}>
              <h3 className="font-serif text-lg text-foreground mb-3">{group.title}</h3>
      ...
      <section id="contact" className="max-w-5xl mx-auto px-6 pb-24 border-t border-border pt-12 scroll-mt-20">
        <p className="section-label mb-4">Contact</p>
        <h2 className="font-serif text-2xl md:text-3xl text-foreground mb-6">Get in touch</h2>
        ...
        {contact.location && <p className="text-sm text-muted-foreground mt-4">{contact.location}</p>}
      </section>
    </div>
  );
}
